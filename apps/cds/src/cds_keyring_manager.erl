	...
	case Shares#{X => Share} of
		AllShares when map_size(AllShares) =:= Threshold ->
			try
				...
				{reply, {ok, unlocked}, unlocked, #state{keyring = UnmarshalledKeyring, masterkey = MasterKey, shares = #{}}}
			catch Error ->
				{reply, {error, Error}, locked, #state{}}
			end;
		MoreShares ->
			{reply, {ok, {more, Threshold - maps:size(MoreShares)}}, locked, StateData#state{shares = MoreShares}, ?UNLOCK_TIMEOUT}
	end;
