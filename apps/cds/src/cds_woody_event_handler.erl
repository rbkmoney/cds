...
filter(#cds_KeyringMetaDiff{} = V) ->
    V;
%% cds_proto storage
filter(#cds_CardData{} = V) ->
    V;
filter(#cds_SessionData{} = V) ->
    V;
filter(#cds_PutCardResult{} = V) ->
    V;
%% identdocstore
...
filter({russian_retiree_insurance_certificate, #identdocstore_RussianRetireeInsuranceCertificate{}} = V) ->
    V;
%% tds
filter(#tds_Token{} = V) ->
    V#tds_Token{content = <<"***">>};
...
filter(#cds_OperationAborted{} = V) ->
    V;
filter(#cds_InvalidCardData{} = V) ->
    V;
filter(#cds_CardDataNotFound{} = V) ->
    V;
filter(#cds_SessionDataNotFound{} = V) ->
    V;
%% identdocstore exceptions
filter(#identdocstore_IdentityDocumentNotFound{} = V) ->
    V;
%% tds exceptions
filter(#tds_TokenNotFound{} = V) ->
    V;
%% woody errors
